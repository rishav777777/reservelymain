...
export interface Reservation {
  ...
  status:   'pending' | 'confirmed' | 'rejected' | 'arrived' | 'no_show' | 'completed' | 'cancelled'
  initials: string
}
...
const STATUS_STYLE: Record<string, { bg: string; border: string; textColor: string }> = {
  confirmed: { bg: 'rgba(27,122,67,0.1)',    border: '1px solid rgba(27,122,67,0.15)',    textColor: '#1B7A43' },
  rejected:  { bg: 'rgba(168,41,41,0.08)',   border: '1px solid rgba(168,41,41,0.12)',    textColor: '#A82929' },
  arrived:   { bg: 'rgba(13,71,43,0.12)',    border: '1px solid rgba(13,71,43,0.2)',      textColor: '#0D472B' },
  no_show:   { bg: 'rgba(120,100,30,0.1)',   border: '1px solid rgba(120,100,30,0.2)',    textColor: '#7a6020' },
  completed: { bg: 'rgba(100,120,110,0.08)', border: '1px solid rgba(100,120,110,0.15)', textColor: '#4a6058' },
  cancelled: { bg: 'rgba(168,41,41,0.08)',   border: '1px solid rgba(168,41,41,0.12)',   textColor: '#A82929' },
}

const CARD_BG: Record<string, string> = {
  pending:   'rgba(255,255,255,0.55)',
  confirmed: 'rgba(235,248,240,0.7)',
  rejected:  'rgba(252,238,238,0.6)',
...
const CARD_BORDER: Record<string, string> = {
  ...
  rejected:  '1.5px solid rgba(168,41,41,0.2)',
...
export function DayFeedCard({ reservation, onConfirm, onDecline, onArrived, onNoShow }: DayFeedCardProps) {
  ...
  const { lang } = useLang()
  const tx = dashboardT[lang].dayFeedCard
  ...
  const isDone      = ['rejected', 'no_show', 'completed', 'cancelled'].includes(localStatus)

  const cfg       = STATUS_STYLE[localStatus]
  const statusLabel = localStatus !== 'pending' ? (tx.statusLabels as Record<string, string>)[localStatus] : null

  function handleConfirm() {
    setActioned(true)
    setTimeout(() => { onConfirm(reservation.id, reply); setLocalStatus('confirmed'); setActioned(false) }, 400)
  }
  function handleDecline() {
    setActioned(true)
    setTimeout(() => { onDecline(reservation.id, reply); setLocalStatus('rejected'); setActioned(false) }, 400)
  }
  function handleArrived() {
    setActioned(true)
    setTimeout(() => { onArrived?.(reservation.id); setLocalStatus('arrived'); setActioned(false) }, 300)
  }
  function handleNoShow() {
    setActioned(true)
    setTimeout(() => { onNoShow?.(reservation.id); setLocalStatus('no_show'); setActioned(false) }, 300)
  }
  ...
  return (
    <div
      ...
      style={{
        ...
        boxShadow: isPending ? '0 4px 32px rgba(28,35,31,0.1), 0 1px 4px rgba(28,35,31,0.06)' : '0 2px 16px rgba(28,35,31,0.07)',
    ...
    >
      ...
      {!isPending && statusLabel && cfg && (
        <div
          className="flex items-center justify-between px-6 py-2.5"
          onClick={() => isConfirmed && setExpanded(v => !v)}
          style={{ background: cfg.bg, borderBottom: cfg.border, cursor: isConfirmed ? 'pointer' : 'default' }}
        >
          <div className="flex items-center gap-2">
            ...
            {(localStatus === 'rejected' || localStatus === 'cancelled') && <X size={14} strokeWidth={2.5} style={{ color: cfg.textColor }} />}
            {localStatus === 'completed'  && <Check     size={14} strokeWidth={2.5} style={{ color: cfg.textColor }} />}
            <span style={{ fontFamily: "'DM Sans', sans-serif", fontSize: '12px', fontWeight: 700, color: cfg.textColor, letterSpacing: '0.1em', textTransform: 'uppercase' }}>
              {statusLabel}
            </span>
          </div>
          {isConfirmed && (
      ...
      >
        <div className="flex items-center justify-center rounded-2xl shrink-0" style={{ width: 56, height: 56, background: palette.bg, fontFamily: "'DM Sans', sans-serif", fontSize: '18px', fontWeight: 700, color: palette.text }}>
        ...
        <div className="flex items-center justify-center rounded-2xl shrink-0" style={{ width: 56, height: 56, background: 'rgba(13,71,43,0.08)', border: '1px solid rgba(13,71,43,0.12)' }}>
          <div className="flex flex-col items-center">
            ...
            <span style={{ fontFamily: "'DM Sans', sans-serif", fontSize: '9px', fontWeight: 600, color: '#5a6b60', letterSpacing: '0.08em', textTransform: 'uppercase', marginTop: '1px' }}>{tx.prs}</span>
          </div>
        </div>
      </div>
            ...
            <>
              ...
              <div className="flex gap-3 px-6 pb-6">
                <button onClick={handleDecline} className="flex-1 flex items-center justify-center gap-2.5 rounded-xl active:scale-95"
                  ...
                  <span style={{ fontFamily: "'DM Sans', sans-serif", fontSize: '17px', fontWeight: 700, color: '#fff', letterSpacing: '0.03em' }}>{tx.decline}</span>
                </button>
                <button onClick={handleConfirm} className="flex-1 flex items-center justify-center gap-2.5 rounded-xl active:scale-95"
                  ...
                  <span style={{ fontFamily: "'DM Sans', sans-serif", fontSize: '17px', fontWeight: 700, color: '#fff', letterSpacing: '0.03em' }}>{tx.confirm}</span>
                </button>
              </div>
            </>
          )}
          ...
          {isConfirmed && (
            <div className="flex gap-3 px-6 pb-6 pt-1">
              <button onClick={handleNoShow} className="flex-1 flex items-center justify-center gap-2 rounded-xl active:scale-95"
                ...
                <span style={{ fontFamily: "'DM Sans', sans-serif", fontSize: '14px', fontWeight: 700, color: '#7a6020', letterSpacing: '0.02em' }}>{tx.noShow}</span>
              </button>
              <button onClick={handleArrived} className="flex-1 flex items-center justify-center gap-2 rounded-xl active:scale-95"
                ...
                <span style={{ fontFamily: "'DM Sans', sans-serif", fontSize: '14px', fontWeight: 700, color: '#0D472B', letterSpacing: '0.02em' }}>{tx.arrived}</span>
              </button>
            </div>
          )}

...
export function ActionButtons({ status, loading, onAction }: ActionButtonsProps) {
  const { lang } = useLang()
  const tx = dashboardT[lang].actionButtons
  ...
  const ACTIONS_BY_STATUS: Record<ReservationStatus, ActionConfig[]> = {
    pending: [
      { label: tx.confirm,     targetStatus: 'confirmed', variant: 'primary', icon: <Check size={12} /> },
      { label: tx.reject,      targetStatus: 'rejected',  variant: 'danger',  icon: <X size={12} /> },
    ],
    confirmed: [
      { label: tx.markArrived, targetStatus: 'arrived',   variant: 'primary', icon: <UserCheck size={12} /> },
      { label: tx.cancel,      targetStatus: 'cancelled', variant: 'ghost',   icon: <XCircle size={12} /> },
      { label: tx.noShow,      targetStatus: 'no_show',   variant: 'ghost',   icon: <AlertCircle size={12} /> },
    ],
    arrived: [
      { label: tx.completed,   targetStatus: 'completed', variant: 'primary', icon: <CheckCircle size={12} /> },
      { label: tx.noShow,      targetStatus: 'no_show',   variant: 'ghost',   icon: <AlertCircle size={12} /> },
    ],
    completed: [],
    rejected:  [],
